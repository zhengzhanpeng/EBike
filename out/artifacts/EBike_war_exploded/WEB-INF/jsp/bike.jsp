            ...
            <div class="mui-scroll">
                <ul class="mui-table-view">
                    <c:forEach items="${list}" var="logM">
                        <li class="mui-table-view-cell">
                            <div class="bikeId mui-hidden">${logM.id}</div>
                            <span class="bikeType">${logM.bikeType}</span>
...
<script>
    ...
    mui.ready(function () {
        ...
        //点击编辑按钮
        $(".set").click(function () {
            var bikeId = $(this).parent().children(".bikeId").text();
            var bikeType = $(this).parent().children(".bikeType").text();
            layer.prompt({title: '请输入修改后名称'}, function (pass, index) {
                $.ajax({
                    url: "setBike.do"
                    , type: "get"
                    , data: {"id": bikeId, "bikeType": pass}
