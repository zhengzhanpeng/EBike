            ...
            <div class="mui-scroll">
                <ul class="mui-table-view">
                    <c:forEach items="${colorList}" var="color">
                        <li class="mui-table-view-cell">
                            <div class="colorId mui-hidden">${color.id}</div>
                            <span class="bikeColor">${color.bikeColor}</span>
...
<script>
    ...
    mui.ready(function () {
        ...
        //点击编辑按钮
        $(".set").click(function () {
            var colorId = $(this).parent().children(".colorId").text();
            var bikeColor = $(this).parent().children(".bikeColor").text();
            layer.prompt({title: '请输入修改后名称'}, function (pass, index) {
                $.ajax({
                    url: "setColor.do"
                    , type: "get"
                    , data: {"id": colorId, "bikeColor": pass}
